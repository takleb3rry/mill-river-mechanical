# Mill River Mechanical — Claude SMB demo kit
...
The company also has a public demo website.

Mill River Mechanical is invented. No person, customer or figure in this kit is real.

**Demo website:** _URL added after first deploy_

**Time to deploy the kit: about 1.5 hours.** Accounts take 20 minutes, Claude's connector phase about 30 (Drive is the slowest part),
and the manual steps another 30. After that, re-seeding for a new date takes about 45 minutes.

**Get the files:** green **Code** button → **Download ZIP**.

## What's in this repo

| Item | What it is |
| --- | --- |
| `demo-kit/01 Company bible.md` | The company, the story, the demo run sheet, the trainer flags. Read it once. |
| `demo-kit/02 Seed data.md` | Every record, as day-offsets from S. Change amounts or names here. |
| `demo-kit/03 Setup/` | Text for Claude settings and the Gmail signature. |
| `demo-kit/05 Drive payload/` | Contracts, CLAUDE.md files and CSVs that go into Drive. |
| `demo-kit/06 Deploy/generate.py` | Takes your demo Monday and writes a dated pack: Claude specs, Zoho import files, Drive CSVs, and your manual steps with exact amounts. |
| `brand/` | Logo, logo mark, palette and four job-site photos at full resolution. See `brand/README.md`. |
| `website/` | The one-page demo website. See `website/README.md`. |
| `vercel.json` | Tells Vercel to serve only `website/`. Nothing else in the repo is published to the site. |
...
3. **Generate the dated pack.** Run `python3 "demo-kit/06 Deploy/generate.py" YYYY-MM-DD` (Python 3, no installs needed). You get
   `demo-kit/06 Deploy/out-YYYY-MM-DD/`. If you don't have Python, attach the kit to Cowork and ask Claude to run it.
...
- **Google:** set Dana's Gmail signature, and drag `brand/logo.png` and the four photos in `brand/photos/` into Drive `00 Brand/`.
...
| The Drive connector can't carry images (logo 0.5 MB, photos ~2.3 MB), and a binary upload came back corrupted once. | Images are dragged in by hand from `brand/`. The spec checks every upload's size and retries on a mismatch. |
...
A one-page static site for the company, deployed by Vercel from the `website/` folder on every push to `main`. It has no build
step and no backend, and a `noindex` tag keeps it out of search results. Page details and brand rules are in `website/README.md`.